...
int main(int argc, char* argv[])
{
	...
	uint32_t *INPUT = (uint32_t *)AllocateMemory(sizeof(uint32_t) * INPUT_HEIGHT * PADDED_WIDTH);
	uint32_t *BIN = (uint32_t *) AllocateMemory(sizeof(uint32_t) * HISTO_HEIGHT * HISTO_WIDTH);
	uint8_t *OUTPUT = (uint8_t *) AllocateMemory(sizeof(uint8_t) * HISTO_HEIGHT * HISTO_WIDTH);
	
	ToDeviceMemory(INPUT, &(input[0][0]),sizeof(uint32_t) *  PADDED_WIDTH * INPUT_HEIGHT);
    /* End of setup code */

    /* This is the call you will use to time your parallel implementation */
    TIME_IT("opt_2dhisto",
            1000,
            opt_2dhisto(INPUT,  INPUT_HEIGHT, INPUT_WIDTH, OUTPUT, BIN );)
    /* Include your teardown code below (temporary variables, function calls, etc.) */
	ToHostMemory(kernel_bins, OUTPUT, sizeof(uint8_t) * HISTO_HEIGHT * HISTO_WIDTH);
	freeMemory(INPUT);
	freeMemory(OUTPUT);
	freeMemory(BIN);
	...
	printf("%u  %u \n",gold_bins[j],kernel_bins[j]);// Values for CPU implementation and GPU implementation
	}
    for (int i=0; i < HISTO_HEIGHT*HISTO_WIDTH; i++){
	ctr++;//Iteration at which loop exits
